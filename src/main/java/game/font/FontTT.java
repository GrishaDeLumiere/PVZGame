 ...
 * принимает BufferedImage и преобразует его в текстуру. Если изображение
 * не является "степенью двойки" (power of 2), TextureLoader делает его 
 * степенью двойки и устанавливает соответствующие текстурные координаты.
 *
 * Изменено Grisha: добавлена поддержка альфа-канала для работы с прозрачностью и поддержка русских букв.
 */
public class FontTT {
	...
	public void drawText(String whatchars, float size, float x, float y, float z, Color4f color, float rotxpass, float rotypass, float rotzpass, boolean centered) {
	    ...
	    GL11.glColor4f(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha());  // Устанавливаем цвет с альфа-каналом
	...
	public void drawTextWithShadow(String whatchars, float size, float x, float y, float z, Color4f color, 
            ...
            GL11.glColor4f(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha());  // Устанавливаем основной цвет
    ...
    for (float offsetX = -outlineWidth; offsetX <= outlineWidth; offsetX += outlineWidth / 2) {
        for (float offsetY = -outlineWidth; offsetY <= outlineWidth; offsetY += outlineWidth / 2) {
            if (offsetX == 0 && offsetY == 0) continue; // Пропускаем центр (основной текст)
            ...
            for (int i = 0; i < whatchars.length(); i++) {
                ...
                k = charData.charnum;
                drawtexture(charactersp[k], fontsizeratio, offsetTotalWidth, offsetY, outlineColor, rotxpass, rotypass, rotzpass);
